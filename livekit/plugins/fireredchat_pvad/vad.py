...
SLOW_INFERENCE_THRESHOLD = 0.01  # late by 200ms
...
class SpeakerEmbExtractor(object):
    def __init__(self, ckpt_path=None, device="cpu"):
        if ckpt_path is None:
            ckpt_path = str(Path(__file__).parent.resolve()) + "/spkrec-ecapa-voxceleb/"
        self.device = device
        if device == "cpu":
            device_str = "cpu"
        else:
        ...
        self.classifier = EncoderClassifier.from_hparams(
            source=ckpt_path, savedir=ckpt_path, run_opts={"device": device_str}
        )
...
class VadProcessor:
    def __init__(self):
        ...
        self._session = ort.InferenceSession(os.path.join(os.path.dirname(__file__), "resources/pvad.onnx"), providers=["CPUExecutionProvider"], sess_options=opts)
        self._spk_extractor = SpeakerEmbExtractor(os.path.join(os.path.dirname(__file__), "resources/spkrec-ecapa-voxceleb"))
        self.window_size_samples = 160
    ...
    def __call__(self, wav_np):
        if wav_np.shape[0] != 160:
            logger.debug(f"vad input shape error: {wav_np.shape}")
            return 0.0
        ...
        raw_prob = outputs[1][0].tolist()[0] # 1
...
class VAD(agents.vad.VAD):
    ...
    def load(
        ...
        *,
        ...
        max_buffered_speech: float = 20.0,
        ...
        if sample_rate != 16000:
            raise ValueError("VAD only supports 16KHz sample rates")
...
class VADStream(agents.vad.VADStream):
    def __init__(self, vad: VAD, opts: _VADOptions, model) -> None:
        super().__init__(vad)
        ...
        self._speaker_emb_updated = False
        self._ecapa_resampler = None
        self._ecapa_window_size = 80000 # take 1 second for spk embedding calculation
    ...
    def update_speaker(
        ...
        if not self._speaker_emb_updated:
            ...
            inference_data_f32 = np.array(frame.data[:self._ecapa_window_size], dtype=np.float32) / 32768.0
            audio = torch.from_numpy(inference_data_f32).unsqueeze(0)

            # Run inference
            self._model.spkemb = self._model._spk_extractor.get_embedding(audio)
            logger.info(
                "pvad: embedding updated"  # noqa: E501
            )
                ...
                if p >= self._opts.activation_threshold:
                    ...
                    if not pub_speaking:
                        if speech_threshold_duration >= self._opts.min_speech_duration:
                            logger.info(f"VAD: start of speech")
                            pub_speaking = True
                            pub_silence_duration = 0.0
